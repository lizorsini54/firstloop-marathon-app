...
**Pre-prod and prod use Clerk's Development/Production instance split within one Application, not separate keys and not a second Application**: discussed with the user, and refined once we noticed Clerk's own dashboard already offers this — a Clerk Application has a Development instance and a Production instance, each with its own keys and its own user pool, switchable from one dropdown. That's exactly the isolation we wanted (pre-prod's Playwright test account and any seed data never appear in prod's user pool) without the extra unnecessary step of standing up an entirely separate Application, which would just need its *own* production instance added anyway to be a real prod environment.
